...
async function getFlightsResults(params: any) {
    const { origin, destination, departureDate, returnDate }: IGetFlightsParams = params;
...
async function getFlightsFromPage(page: Page) {
    return await page.evaluate(() => {
        ...
        return flights.map((el) => {
            ...
            const getTextContent = (selector: string) =>
                el.querySelector(selector)?.textContent?.trim() || `No ${selector.split(' ')[1]}`;
